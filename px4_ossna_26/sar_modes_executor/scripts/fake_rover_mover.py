...
start_delay seconds once at startup, then loops straight-line driving
(straight_duration seconds) and circling (circle_radius, num_circle_turns
turns) back to back until the node is stopped, so the SAR formation modes
have a real moving target to track instead of a static pose.
"""
import math
...
class FakeRoverMover(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('linear_speed', 0.5)        # m/s, held constant through both moving phases
        ...
        # Angular rate that makes the circle phase trace circle_radius at linear_speed.
        self._circle_omega = self._speed / self._circle_radius if self._speed > 0.0 else 0.0
        ...
        self.get_logger().info(
            ...
            f'Holds {self._start_delay:.1f}s, then loops: straight {self._straight_duration:.1f}s, '
            f'circle r={self._circle_radius:.1f}m for {self._num_circle_turns:.1f} turns, repeat.'
        )
